...
def node_name(source_code_bytes: bytes, ast: Node) -> Optional[str]:
    ...
    if ast.type in {"variable_declarator", "formal_parameter"}:
        ...
        expr_ast = (
            expr_ast
            if expr_ast.next_named_sibling is None
            else expr_ast.next_named_sibling
        )
        ...
        return (
            None
            if identifier_ast is None
            else node_name(source_code_bytes, identifier_ast)
        )
        ...
        return source_code_bytes[
            ast.children[0].start_byte : ast.children[0].end_byte
        ].decode("utf8")
